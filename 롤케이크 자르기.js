...
//철수에게 모든 케이크를 주고 한조각씩 동생에게 주면서 토핑을 비교한다

function solution(topping) {
    let 철수 = new Map()
    let 동생 = new Map()
    var result = 0
    topping.map(a => 철수.set(a, (철수.get(a) || 0) + 1))
    for (let i = topping.length-1; i>=0; i--){
        철수.set(topping[i], (철수.get(topping[i]) -1 )) 
        if(철수.get(topping[i]) === 0 ) 철수.delete(topping[i])
        동생.set(topping[i], ((동생.get(topping[i])||0) +1 ))

        //토핑 개수를 세는 시간을 줄여야한다...
        // if([...철수].length === [...동생].length) {
        //     result ++
        // }

        if(철수.size === 동생.size) result ++
        //공식 문서를 참고하니 Map의 경우 Map.size를 활용하면 item의 개수를 쉽게 알아낼수 있었다.
    }
