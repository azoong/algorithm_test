...
function solution(order) {
    var answer = 0
    let stack = []
    let i = 0   //배열의 앞쪽에서 수정이 생길 경우 효율성이 떨어지기에 변화가 잇을시 인덱스만 바꿔준다.
    let j = 0   //i는 arr의 인덱스, j는 order의 인덱스
    let arr = Array.from(order, (_ , x) => x+1) // [1,2,3,4,5 ....]
    
    while (i <= arr.length){
        if(!order[j]) break
        else if(arr[i] == order[j]) { 
            answer ++
            i++
            j++
        }else if(order[j] == stack.slice(-1)){  //스택에서 마지막 인덱스 삭제해주고, order인덱스 +1
            answer ++
            stack.pop()
            j++
        }else{ //값이 다를 경우 arr인덱스만 +1 해주고 스택으로 푸쉬해준다.
            stack.push(arr[i])
            i ++
        }
    }
